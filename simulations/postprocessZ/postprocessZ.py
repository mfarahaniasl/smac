...
emfile = "postprocessZ/micaZ_energy_model.txt"
outFile = "result/EnergyPowerCurses.txt"
diedFile = open("result/nodeDied.txt","w")
...
eff_table_filename = "postprocessZ/EfficiencyTable.csv" # table of battery efficiency vs current load
rec_table_filename = "postprocessZ/RecoveryProbabilityTable.csv" # table of recovery probability vs battery remaining capacity
...
battery_total_charge = 2000.0 # mAh 2000.0
...
def parse_args():
    ...
    while argv:
        ...
        if a == "--help":
            ...
            powercurses = 1
        elif a == "--powercursesToFile":
            powercurses_toFile = 1
	    argv=argv[1:]  # Consume this argument
            if not argv:
                quit(1)
            outFile = argv[0]  # Get the filename parameter
            ...
            if not argv:
                quit(1)
            battery_total_charge = float(argv[0])#int
        elif a == "--effTable":
            argv = argv[1:] # Consume this argument
            if not argv:
                quit(1)
...
def initstate():
    ...
    global efftable, rectable, battery_total_charge, mote_died, pcOut # AOC: added battery
    read_em()
    ...
    for mote in range(maxmotes):
        ...
        prev_battery[mote] = {}
        battery_max[mote] = {}
        battery[mote] = battery_total_charge if mote<>0 else 0
        mote_died[mote] = -1
        prev_battery[mote] = battery_total_charge if mote<>0 else 0
        battery_max[mote] = battery_total_charge if mote<>0 else 0
        prev_current[mote]={}
        for k in totals:
...
def handle_event(l):
    ...
    #if powercurses_toFile:
        #pcOut.writelines("POWERCURSES,%.6f,%d,%.2f\n" % (time/simfreq,mote,battery[mote]/battery_total_charge*100))
...
def print_summary():
    ...
    for mote in range(maxseen+1):
        #if prettyprint:
            #diedFile = open("result/nodeDied.txt","w")
	    diedFile.writelines("%d;\t%.1f;\tMote ran out of battery\n" %(mote, mote_died[mote]/simfreq ))
...
if __name__=='__main__':
    ...
    print "Trace file loaded, size=%i",lineno
    if summary:
        print_summary()
